...
#+BEGIN_SRC R :results output :exports both :session *R* :cache no 
melanoma= base::transform(melanoma, 
           thick2=thick^2, 
           lthick=log(thick) ) 
dhead(melanoma)
#+END_SRC
...
using the dby funtion 

#+BEGIN_SRC R :results output :exports both :session *R* :cache no 
 dby(melanoma,status==1)          <- list(z=NA)
 dby(melanoma,thick~+1|status!=1) <- list(z=log)
 dsummary(a,z~I(status!=1))

 a <- dby(melanoma,thick~+1,y=log,y1=log)
 dby(a,~thick+status|status!=1) <-  list(y= function(x) x[,1]+x[,2])
 dby(a,thick+status~1|status!=1) <- list(y1=function(x) x[,1]+x[,2])
 dsummary(a,'y*'+thick~status)
#+END_SRC

#+RESULTS:
#+BEGIN_example
Error in is.vector(data) : object 'a' not found
status: 1
       y               y1            thick       
 Min.   :3.466   Min.   :3.466   Min.   :  32.0  
 1st Qu.:5.412   1st Qu.:5.412   1st Qu.: 224.0  
 Median :5.869   Median :5.869   Median : 354.0  
 Mean   :5.783   Mean   :5.783   Mean   : 431.1  
 3rd Qu.:6.182   3rd Qu.:6.182   3rd Qu.: 484.0  
 Max.   :7.463   Max.   :7.463   Max.   :1742.0  
------------------------------------------------------------ 
status: 2
       y                y1             thick       
 Min.   :  12.0   Min.   :  12.0   Min.   :  10.0  
 1st Qu.:  83.0   1st Qu.:  83.0   1st Qu.:  81.0  
 Median : 137.5   Median : 137.5   Median : 135.5  
 Mean   : 226.5   Mean   : 226.5   Mean   : 224.5  
 3rd Qu.: 288.0   3rd Qu.: 288.0   3rd Qu.: 286.0  
 Max.   :1290.0   Max.   :1290.0   Max.   :1288.0  
------------------------------------------------------------ 
status: 3
       y                y1             thick       
 Min.   :  19.0   Min.   :  19.0   Min.   :  16.0  
 1st Qu.: 132.0   1st Qu.: 132.0   1st Qu.: 129.0  
 Median : 229.0   Median : 229.0   Median : 226.0  
 Mean   : 374.8   Mean   : 374.8   Mean   : 371.8  
 3rd Qu.: 583.0   3rd Qu.: 583.0   3rd Qu.: 580.0  
 Max.   :1259.0   Max.   :1259.0   Max.   :1256.0
#+END_example

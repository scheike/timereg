...
In addition we note that when looking at one pair that has been ascertained the information about then
\begin{align*}
P(Y_i=o_1,Y_j=o_2 | Y_i+Y_j \geq 1) & = \sum_{k=1}^2 P(Y_i=o_1,Y_j=o_2 | Y_i+Y_j = k )  P(  Y_i + Y_j =k | Y_i + Y_j \geq 1 ).
\end{align*}
where \( o_1+ o_2  \geq 1 \).  Now the dependence is present in the how the dependence parameter leads to the probabilities 

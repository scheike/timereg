...
We here mention to utility functions 
 - tie.breaker for breaking ties among jump-times which is expected in the functions below.
 - count.history that counts the number of jumps previous for each subject that is 
   $N_1(t-)$ and $N_2(t-)$. 
...
** Looking at other simualtions with dependence 

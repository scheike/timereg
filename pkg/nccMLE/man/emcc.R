...
betait=100,detail=0,emvar=1)
print(c(out$beta,diag(out$var)^.5)); # estimates related to Z

out<-em.ncc(Zobs,Tobs,status,tau,nno,Nit=100,beta=c(-.2,.2),em.dif=0.001,
betait=50,detail=2,emvar=1)
print(c(out$beta,diag(out$var)^.5)); # estimates related to Z

###out2<-em.ncc(Zobs,Tobs,status,tau,nno,Nit=100,beta=out$beta,
###betait=100,detail=0,emvar=2)
###print(c(out2$beta,diag(out2$var)^.5)); # estimates related to Z

gem<-rbind(gem,
c(out$beta,diag(out$var)^.5,out2$beta,diag(out2$var)^.5) )
}

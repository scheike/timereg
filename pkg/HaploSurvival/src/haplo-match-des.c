...
//  lamba_i(t) = X_i(h)^T a(t) \exp(Z_i(h)^T \beta ) 
//
void haplosurvmatch(
// {{{
...
// {{{ memory allocation 1
  ...
  vector *RRv,*vhaplo1,*vhaplo2,*XtempScore,*tempScore,
	 *scorei[(*antpers)],*Xscorei[(*antpers)]; 
  int nphm1=nph[0]-1,pp=0,t,c,robust=1,pers=0,i,j,k,l,s,it,count,pmax,
      *risk=calloc(antpers[0],sizeof(int)),nap,
      *ipers=calloc(Ntimes[0],sizeof(int));
  int amount[1],dimpar,fixmax,c2; 
  int haplotypeP[2],haplotypeD[2]; 
  int c1Dstart=0,c1D,c1P,c2D,c2P,jD,jP; 
  double phD,phP;
  ...
  double norm_rand(); 
  void GetRNGstate(),PutRNGstate(),haplodesignMM(),genoLogLikeHp(); 
  double surv,shaz,dsurvbeta,dsurvA,dhazbeta,dhazA; 
  double zihbeta,xihAt,maxdelt=10,taelpers=0,naevnpers=0; 
...
// Rprintf("dimpar= %ld pg=%ld dimzih= %ld dimhap=%ld px= %ld dimxih=%ld \n",dimpar,*pg,*dimzih,*dimhap,*px,*dimxih); 
  ...
  malloc_vecs(*antpers,&RRv,&Lplamt,&dlamt,&plamt,&lamt,&zcol,&Gbeta,&one,&offset,NULL); 
...
// }}}
  ...
  if (*fixhaplofreq==0)  {
   ...
   for (j=0;j<*nph-1;j++) {VE(Uf,j)=scoregenoC[j];}
       ...
       // print_vec(U); print_vec(Uhap); print_mat(hapDes); 

    vM(hapDes,Uf,Uhap); MtA(hapDes,S1f,AA); MxA(AA,hapDes,S1hap);
    // print_vec(Uhap); print_mat(S1hap); 
 ...
 // }}}
    ...
    for (s=1;s<*Ntimes;s++){// starts with 1 because times[0] is 0 (start.time)
       ...
       time=times[s]; mat_zeros(ldesignX); mat_zeros(ldesignG);
	   ...
	   count=count+1; } else {risk[id[c]]=0;} 
       }
       ipers[s]=pers; 
 ...
 //   Rprintf("start af antpers loop der laver design \n"); 
    for (i=0;i<*antpers;i++) {

   // initializing  {{{
       naevn=0; tael=0; VE(RRv,i)=1.0; 
   ...
   if ( s==0) { // print test {{{
      Rprintf("s,i  %d %d \n",s,i); 
      Rprintf("jD, jP  %d %d %d %d \n",jD,jP,c1D,c1P); 
      Rprintf(" Donor haplotype  %d %d %d \n",i,haplotypeD[0],haplotypeD[1]); 
      Rprintf(" Patient haplotype  %d %d %d \n",i,haplotypeP[0],haplotypeP[1]); 
      print_vec(zi); print_vec(zih);
      print_vec(xi); print_vec(xih);
   Rprintf(" frequency of haplotypes %lf %lf \n",haplofreq[oh[c1D]],haplofreq[oh[c1D+1]]);
   Rprintf(" frequency of haplotypes %lf %lf \n",haplofreq[oh[c1P]],haplofreq[oh[c1P+1]]);
   print_vec(beta); 
   }
   // print test }}}

   // basic quantities for intensity {{{
   vec_star(zih,beta,zih2); zihbeta=vec_sum(zih2); 
   vec_star(xih,tmpv1,xih2); xihAt=vec_sum(xih2); xihAt=max(0,xihAt); 
   RR=exp(zihbeta);  VE(RRv,i)=RR; 
   ...
   shaz=exp(-xihAt*RR)*exp(zihbeta)*ph; naevn=naevn+surv; tael=tael+shaz; 
   vec_add_mult(rowXh,xih,shaz,rowXh); 
   if (pp==1) Rprintf("shaz surv ===== %d %d %d %lf %lf %lf %lf %lf  \n",s,i,j,shaz,surv,naevn,tael,xihAt); 
   if (pp==1) { Rprintf("%lf %d %lf \n",ph,risk[i],exp(zihbeta));  print_vec(xih); print_vec(rowXh); }
   // }}}
   ...
   if ((*fixhaplofreq==0) && (*twostage==0) && (risk[i]==1) ) {
  ...
  for (k=0;k<*dimzih;k++) {
   ...
   dsurvA=(-exp(zihbeta))*surv; dhazA=exp(zihbeta)*dsurvA; 
           if (risk[i]==1) {
           for (k=0;k<*dimxih;k++) {
	 ...
	 c1P+=2;  
	   // }}}
         ...
         } /* for (jP=0;jP<nphpp[i];j++)  */ 
	 c1D+=2;  
       } else { c1D+=2; } // this needs to work with jere

       // set up  X and Z matrices {{{
      if (risk[i]==0) {naevn=1; tael=1;}
      weight[i]=naevn/tael;  
      if (i==pers && risk[i]==1) {naevnpers=naevn; taelpers=tael;}
      if (risk[i]==1) {
	 ...
	 extract_row(cdesX,i,xih1); 
	    ...
	    if (i==pers) for (j=0;j<*dimzih;j++) VE(DthetaZdN[k],j)=VE(rowZh,j); 
            VE(rowZtheta1,k)=VE(rowZbeta1,k); VE(rowZtheta2,k)=VE(rowZbeta2,k); 
	    if (i==pers) {VE(rowZtheta1dN,k)=VE(rowZbeta1,k);VE(rowZtheta2dN,k)=VE(rowZbeta2,k);}
         }
	 }
	 ...
	 scl_vec_mult(1/naevn,rowZnu2,vhaplo2); 
	    ...
	    if (i==pers) {VE(rowZtheta1dN,*dimzih*(*fixbeta==0)+j)=VE(rowZnu1,j); 
      ...
      // set up matrices with joint derivatives 
       if (*fixbeta==0) {
          ...
          if (risk[i]==1) {
             ...
             for (c=0;c<*dimxih;c++)  ME(DthetaX[k],i,c)=0; 
	  }
       }
       ...
       for (k=0;k<*dimhap;k++) {
         if (risk[i]==1) {
            ...
            for (c=0;c<*dimxih;c++) ME(DthetaX[*dimzih*(*fixbeta==0)+k],i,c)=0;
	 }
       }
       ...
       if (risk[i]==1)  {
          ...
          for (c=0;c<*dimxih;c++)  ME(DAX[k],i,c)=0; 
       }
       }
...
//    Rprintf("ude af antpers loop der laver design \n"); 
    ...
    // print test {{{
      ...
      // baseline computed  {{{
      MtA(cdesX,cdesX2,A); invert(A,AI); scl_mat_mult(1.0,AI,S0tI[s]);   
      extract_row(cdesX2,pers,xih); Mv(AI,xih,dA); scl_vec_mult(1.0,dA,dAt[s]); 
     ...
     // Second derivative S   {{{
      for (k=0;k<dimpar;k++) {
          ...
          extract_row(DthetaX[k],pers,rowXh); 
          scl_vec_mult(weight[pers],rowXh,xih1); // NY version med W 
          dummy=(VE(rowZtheta2dN,k)*taelpers-VE(rowZtheta1dN,k)*naevnpers)/(taelpers*taelpers); // NY 
          extract_row(cdesX,pers,rowXh);  
   ...
   // variance and other things  {{{
	...
	if (*fixbeta==0 || *fixhaplofreq==0) {
	   ...
	   //   Rprintf(" YIt %ld \n",j); print_mat(S0tI[s]); print_mat(DthetaS0[j]); 
              ...
              extract_row(DthetaX[j],pers,xih1); 
	      scl_vec_mult(weight[pers],xih1,xih1); // NY version med W 
              extract_row(cdesX,pers,rowXh);  // NY version med W
    ...
    // Newton-Raphson step  {{{
     ...
     if (*lm>0) { // Levenberg-Marquards algorithm 
        if (*detail>=1)  Rprintf("Levenberg-Marquardt step, sumscore, maxdelt %d %lf %lf \n",it,sumscore,maxdelt);
      ...
      if (*detail>=1)  Rprintf(" Newton-Raphson step, sumscore %d %lf %lf \n",it,sumscore,maxdelt);
...
//	    print_vec(vtheta1); print_vec(vtheta2); 
	   ...
	   if (i==ipers[s]) {vec_add(vtheta1,W2[i],W2[i]);}
	...
	for (t=1;t<=s;t++) { 
	   ...
	   if (i==ipers[t]){vec_add(rowXh,W3[i],W3[i]);}
	}
...
void designevalM(double *x, double *hD, double *hP, SEXP f, double *res, SEXP rho, int dimx,int dimres) // {{{
{
  ...
  defineVar(install("x"), mkansvM(x,dimx),rho);
  defineVar(install("hd"), mkansvM(hD,2),rho);
  defineVar(install("hp"), mkansvM(hP,2),rho);
  ans=eval(f,rho);
  PROTECT(ans);

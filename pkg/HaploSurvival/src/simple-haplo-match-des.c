...
//
void simplehaplosurvmatch(
// {{{
times,Ntimes,designX, nx,px,designG,
ng,pg,antpers, start,stop,betaS,
Nit,cu,vcu, loglike,Iinv,Vbeta,
detail,sim,antsim, Rvcu,RVbeta,test,
testOBS,Ut,simUt, Uit,wscore,idtimes,
status,score,dhatMit, dhatMitiid,retur,sym,
nph,oh,nphpp, haplopars,fixbeta,fixhaplofreq,
dimzih,dimxih,rho, scoregeno,d2lgeno, survhaploscore,
...
// {{{ memory allocation 1
  ...
  matrix *dW3t[*antpers],*W3t[*antpers],*W4t[*antpers],*W2t[*antpers],*AIxit[*antpers],*Uti[*antpers],*tmp4;
  ...
  int amount[1],dimpar,fixmax,c2,haplotypeP[2],haplotypeD[2]; 
  int c1Dstart=0,c1D,c1P,c2D,c2P,jD,jP,toolarge=0; 
  double phD,phP;
...
if (*Ntimes>5000) {
Rprintf("Many jump times: may cause memory problems %d \n",*Ntimes); 
Rprintf("May increase size of Fst in simple-haplo-match-des.c to %d\n",*Ntimes);
Rprintf("then recompile %d\n",*Ntimes); 
}
...
// }}}
 ...
 //   Rprintf("start af antpers loop der laver design \n"); 
    for (i=idtimes[s-1];i<*antpers;i++) {
        ih=indexpersallH[i]; c1D=indexpersnph[i]; 
  ...
  for (k=0;k<*dimzih;k++) {
   ...
   dsurvA=(-RR)*surv; dhazA=RR*dsurvA; 
	 ...
	 extract_row(cdesX,i,xih1); 
         for (k=0;k<*dimzih;k++) {
	      ...
	      VE(DbetaZ2[k],c)=(naevn*VE(DbetaZ2[k],c)-VE(rowZbeta2,c)*VE(rowZbeta2,k))/(naevn*naevn); 
            ...
            vec_subtr(DbetaZ1[k],DbetaZ2[k],rowZh); 
	 ...
	 scl_vec_mult(1/naevn,rowZnu2,vhaplo2); 
      ...
      // set up matrices with joint derivatives 
       ...
       for (k=0;k<*dimxih;k++) {
          ...
          scl_vec_mult(VE(DArowXh,k)/naevn,rowXh,xih2);  
	 ...
	 VE(xih1,l)*VE(xih1,c)* (VE(DArowXh,k)*tael-VE(DArowXh2,k)*naevn)/(tael*tael); 
	     } 
	  }
	     ...
	     extract_row(DthetaX[k],i,xih2); 
	        ...
	        ME(DthetaS0[k],l,c)+= (VE(xih1,l)*VE(xih2,c)+VE(xih2,l)*VE(xih1,c))*(naevn/tael)+
	         VE(xih1,l)*VE(xih1,c)*
		 (VE(rowZtheta2,k)*tael-VE(rowZtheta1,k)*naevn)/(tael*tael); 
	     }
	  }
       // }}}
...
//    Rprintf("ude af antpers loop der laver design \n"); 
      ...
      extract_row(cdesX2,idtimes[s-1],xih); Mv(AI,xih,dA); scl_vec_mult(1.0,dA,dAt[s]); 
   ...
   // variance and other things  {{{
	...
	for (j=0;j<*antpers;j++){
	   ...
	   VE(dLamt[j],s)=vec_prod(xih,dA); 
	   // Rprintf(" %d %d %lf \n",s,j,VE(dLamt[j],s)); 
	   //print_vec(xih); print_vec(dA); 
        }
      }
    // }}}
...
// Rprintf(" hje mig \n") ; 
  ...
  free(scoregenoC); free(d2lgenoC); free(weight); 
  free(ipers); free(risk); 
  // }}}
}

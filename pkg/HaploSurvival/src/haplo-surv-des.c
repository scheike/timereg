...
// {{{ memory allocation 1
  ...
  int nphm1=nph[0]-1,pp=0,t,c,robust=1,pers=0,i,j,k,l,s,it,count,pmax,
      *risk=calloc(antpers[0],sizeof(int)),*ipers=calloc(*Ntimes,sizeof(int)),nap; 
  int amount[1],haplotype[2],dimpar,fixmax,c1,c2; 
  double *weight=calloc(*antpers,sizeof(double)),dtime,time=0,ph,sph,RR,dummy,
...
// }}}
 ...
 // }}}
   ...
   // initializing  {{{
       ...
       for (j=0;j<nphpp[i];j++) if (risk[i]==1) {
   ...
   if ( s==0) { // print test {{{
      ...
      ph=haplofreq[oh[c1]]*haplofreq[oh[c1+1]]*risk[i];
      Rprintf(" %lf %d %d \n",ph,oh[c1],oh[c1+1]); 
   }
   // print test }}}
  ...
  for (k=0;k<*dimzih;k++) {
   ...
   dsurvA=(-exp(zihbeta))*surv; dhazA=exp(zihbeta)*dsurvA; 
	 ...
	 // }}}
         ...
         else c1+=2;  // this needs to work with jere
    ...
    // print test {{{
      ...
      extract_row(cdesX2,pers,xih); Mv(AI,xih,dA); scl_vec_mult(1.0,dA,dAt[s]); // NY version med W 
	 ...
	 MtA(Z,cdesX,ZX); MxA(ZX,AI,ZXAIs[s]);
      ...
      if (s==0) {print_mat(cdesX2); print_mat(Z); print_mat(AI); print_vec(difzzav);}
...
// Rprintf(" hje mig \n") ; 
  ...
  free(scoregenoC); free(d2lgenoC); free(ipers); 
  free(risk); free(weight); 
  // }}}

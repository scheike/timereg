...
// printf("___________ %d %d %d %lf %lf %lf \n",s,ci,id[ci],start[ci],stop[ci],time); 
  ...
  if (*betafixed==0)  {
   ...
   mat_add(dS,S1,S1);  
   St[s]=mat_copy(S1,St[s]); Stg[timegroup[s]]=mat_copy(S1,Stg[timegroup[s]]);
  }
  ...
  if (it==((*Nit)-1)) { 
    ...
    C[s]=mat_copy(Ct,C[s]); mat_copy(Ct,Cg[timegroup[s]]); 

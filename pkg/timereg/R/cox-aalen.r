...
###  print(length(unique(time.group))); 
  ...
  if (stratum==1) strata <- designX %*% (0:(px-1)) else strata <- rep(0,nx); 
###  print(cbind(0:(nx-1),status,fdata$stop,designX,strata))
  if (basesim==1)  sim <- 2; 
###  print(table(strata))
###  print(c(istratum,stratum))
  ...
  nparout <- .C("score", as.double(times), as.integer(Ntimes), 
                ...
                as.integer(sim), as.integer(antsim), as.integer(rani), ## 25
		...
		as.integer(silent),as.integer(strata), PACKAGE = "timereg")
  ## {{{ output
  Iinv <- matrix(nparout[[18]], pg, pg); 
  ...
  Varbeta <- -matrix(nparout[[19]], pg, pg); 
  cumint <- matrix(nparout[[13]], Ntimes, px + 1); 
  vcum <- matrix(nparout[[14]], Ntimes, px + 1)
  gamma <- matrix(nparout[[11]], pg, 1); 
  ...
  nparout <- .C("scorenostrata", as.double(times), as.integer(Ntimes), 
                ...
                as.integer(fdata$antpers), as.double(fdata$start), as.double(fdata$stop),### 11
                as.double(betaS), as.integer(Nit), as.double(cumint), 
                as.double(vcum), as.double(weights), as.integer(mw), 
                as.double(loglike), as.double(Iinv), as.double(Varbeta),  ### 20 
                as.integer(detail), as.double(offsets), as.integer(mof),  

...
void randomcif2cause(times,Ntimes,x, delta,cause,CA1,
		...
		gam2iid,dscore,squarepar,c1fc2
)
...
//    printf(" s j %d  %d\n",s,j); 
   if (KMc[i]<0.001) response=response/0.001; else response=response/KMc[i];
   if (KMc[k]<0.001) response=response/0.001; else response=response/KMc[k];

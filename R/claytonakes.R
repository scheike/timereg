...
##' e <- ClaytonOakes(Surv(lefttime,time,status)~x+cluster(~1,cluster),
...
##' ts <- ClaytonOakes(Surv(time,status)~prop(x)+cluster(~1,cluster),
##'                    data=d2,type="two.stage")
##' ## Marginal=Aalens additive model:
##' ts2 <- ClaytonOakes(Surv(time,status)~x+cluster(~1,cluster),
##'                     data=d2,type="two.stage")
##' ## Marginal=Piecewise constant:
##' e2 <- ClaytonOakes(Surv(time,status)~x+cluster(~-1+factor(z),cluster),
##'                    cuts=c(0,0.5,1,2),data=d2)
##' e2
##' plot(ts)
##' plot(e2,add=TRUE)
##' 
##' e3 <- ClaytonOakes(Surv(time,status)~x+cluster(~1,cluster),cuts=c(0,0.5,1,2),

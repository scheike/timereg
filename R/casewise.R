...
##' outm <- comp.risk(Hist(time,cause)~+1,data=prt,cause=2,times=times)
##' 
...
##' cc <- bicomprisk(Hist(time,cause)~strata(zyg)+id(id),
...
##' outm <-comp.risk(Hist(time,cause)~+1,data=prt,
##'                  cause=2,times=times,max.clust=100)
##' cifmz <-predict(outm,X=1,uniform=0,resample.iid=1)
##' cc <-bicomprisk(Hist(time,cause)~strata(zyg)+id(id),data=prt,

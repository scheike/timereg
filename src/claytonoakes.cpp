...
RcppExport SEXP claytonoakes_cx(SEXP ds, SEXP ts, SEXP es, 
  ...
  colvec clusters = Rcpp::as<colvec>(allcs);
  NumericVector cut = Rcpp::as<NumericVector>(cuts);
  cx_colvec basehaz = Rcpp::as<cx_colvec>(hs);
  cx_colvec mhaz = Rcpp::as<cx_colvec>(mulths);
  cx_colvec theta0 = Rcpp::as<cx_colvec>(var);
  ...
  colvec dt(ncuts-1);
  for (unsigned i=1; i<ncuts; i++) dt(i-1) = cut(i)-cut(i-1);
  cx_colvec L0 = cumsum(basehaz%dt);
  cx_colvec L00(L0.n_rows+1); L00(0)=0;
  for (unsigned i=0; i<L0.n_rows; i++) L00(i+1)=L0(i);
  cx_colvec L1(L0.n_rows); L1(0)=1.0;
  ...
  for (unsigned i=0; i<ncluster; i++) {
	...
	L1(0)=1.0;
	for (unsigned k=1; k<L0.n_rows; k++)
	  L1(k)=exp(curtheta0*multhaz*L0(k-1.0));
	for (unsigned k=0; k<L0.n_rows; k++)
	  L1(k)*=(exp(curtheta0*multhaz*basehaz(k)*dt(k))-1.0)/(curtheta0);
	L1=cumsum(L1);
      }
	...
	logLik += log(multhaz*basehaz(pos)*exp(curtheta0*multhaz*(L00(pos)+basehaz(pos)*(t-cut(pos)))));
	// Ratio of gamma functions
	lgammaratio += log(1.0/curtheta0+(cx)nevent-1.0);
      }

      Haz1 += 1.0/(curtheta0)*exp(curtheta0*multhaz*L00(pos))*
	(exp(curtheta0*multhaz*(basehaz(pos)*(t-cut(pos))))-1.0);
      if (pos>0) Haz1 += L1(pos-1.0);     

      if (e>0) { // Truncation
	it = std::lower_bound(cut.begin(), cut.end(), e);
	epos= int(it-cut.begin())-1.0;	
	Haz0 += 1.0/(curtheta0)*exp(curtheta0*multhaz*L00(epos))*
	  (exp(curtheta0*multhaz*(basehaz(epos)*(e-cut(epos))))-1.0);      
	if (epos>0) Haz0 += L1(epos-1.0);     
      }

      j++;
      if (j==n) break;
    } while (clusters(j)==curcluster);
    logLik += -(1.0/curtheta0+(cx)nevent)*log(1.0+curtheta0*Haz1); // Survival
    logLik += (1.0/curtheta0)*log(1.0+curtheta0*Haz0); // Truncation
    logLik += (cx)nevent*log(curtheta0);
    logLik += lgammaratio;
  }

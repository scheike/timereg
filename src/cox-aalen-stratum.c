...
void scorestratum(times,Ntimes,designX,nx,px,designG,pg,antpers,start,stop,
betaS,Nit,cu,vcu,w,mw,loglike,Iinv,Vbeta,detail,offs,mof,sim,antsim,
rani,Rvcu,RVbeta,
test,testOBS,Ut,simUt,Uit,XligZ,aalen,nb,id,status,wscore,ridge,ratesim,
score,dhatMit,gammaiid,dmgiid,
...
timegroup,maxtimepoint,stratum,silent,strata)
double *designX,*designG,*times,*betaS,*start,*stop,*cu,*w,*loglike,*Vbeta,*RVbeta,*vcu,*offs,*Rvcu,*Iinv,*test,*testOBS,*Ut,*simUt,*Uit,*aalen,*ridge,*score,*dhatMit,*gammaiid,*dmgiid,*Vcovs,*addproc,*gamiid,*biid,*vscore,*weights;
int*covariance,*nx,*px,*pg,*antpers,*Ntimes,*mw,*Nit,*detail,*mof,*sim,*antsim,*rani,*XligZ,*nb,*id,*status,*wscore,*ratesim,*retur,*robust,*addresamp,*resample,*clusters,*antclust,*betafixed,*entry,*exactderiv,*timegroup,*maxtimepoint,*stratum,*silent,*strata;
{ 
  int timing=0; 
  clock_t c0,c1; 
  c0=clock();
...
//  vector *ZXdA[*Ntimes];
  ...
  matrix *dAt; 
  vector *Ui[*antclust]; 
  int stratas=0,cin=0,ci=0,c,pers=0,i=0,j,k,l,s,s1,it,count,pmax,
      *imin=calloc(1,sizeof(int)),
      *cluster=calloc(*antpers,sizeof(int)),
      *ipers=calloc(*Ntimes,sizeof(int)); 
  double S0stratum,S0,RR=1,time=0,ll,lle,llo;
  ...
  if (*robust==1) {
    for (j=0;j<*antclust;j++) { 
     if (*sim>=2)  { malloc_mat(*maxtimepoint,*px,W4t[j]); 
  ...
  malloc_mats(*pg,*pg,&RobVbeta,&tmp2,&dS,&S1,&S2,&SI,&VU,&VUI,NULL); 
  for (c=0;c<(*px)*(*stratum)+1;c++) malloc_mat(*pg,*pg,ZPZ[c]);
...
//  } 
  ...
  {
     if (it>0) {
      vec_zeros(U); mat_zeros(S1); mat_zeros(A); 
      if (*stratum==1)  for(j=0;j<*px;j++) mat_zeros(ZPZ[j]); else mat_zeros(ZPZ[0]); 
      mat_zeros(ZPX); mat_zeros(ZX); 
	  ...
	  for (c=0,count=0;((c<*nx) && (count!=*antpers));c++) 
	  {
	   if (( (start[c]<time) && (stop[c]>=time)) ) {
		...
		S0+=RR*weights[c]; 
	        ...
	        if (j<*pg) {ME(WZ,id[c],j)=weights[c]*designG[j*(*nx)+c];} 
		}
		if (time==stop[c] && status[c]==1) {pers=id[c];stratas=strata[c]; } 
	    ...
	    for(j=0;j<pmax;j++) for(k=0;k<pmax;k++)  {
 	      ...
 	      if ((*exactderiv<2) || (*px==1) || (*stratum==1)) {
                 if ((j<*pg) & (k<*pg)) ME(ZPZ[strata[c]],j,k)+= VE(zi,j)*VE(zi,k)*weights[c]*RR; 
    ...
    while ((stop[ci]<time)  & (ci>=0) ) {
	  ...
	  for(j=0;j<pmax;j++) for(k=0;k<pmax;k++)  {
 	      ...
 	      if ((*exactderiv<2) || (*px==1) || (*stratum==1)) {
                 if ((j<*pg) & (k<*pg)) ME(ZPZ[strata[ci]],j,k)+=entry[ci]*VE(zi,j)*VE(zi,k)*weights[ci]*RR;
...
//	   print_mat(X); 
	   Rprintf("ZPZ matrix, pers %d %d   (at risk)\n",pers,stratas); 
	   print_mat(ZPZ[stratas]); 
...
//    if (fabs(ME(A,k,k))<0.000001)  ME(AI,k,k)=0; else ME(AI,k,k)=1/ME(A,k,k);
    for (k=0;k<*px;k++) ME(AI,k,k)=0; 
//   mat_zeros(AI); 
    if (*px>1) { 
            k=stratas; ME(AI,k,k) = 1/ME(A,k,k);
    }
    else ME(AI,0,0) = 1/ME(A,0,0);
   }

   scale=VE(weight,pers); 

   if ((*stratum==1) && (*px>1) ) {
      S0stratum=ME(AI,stratas,stratas); 
      scl_mat_mult(scale*S0stratum,ZPZ[stratas],ZPZo); 
      scl_mat_mult(scale*S0stratum,ZPX,ZPXo);
//      printf(" %d %lf \n",strata[pers],S0stratum); 
//      print_mat(AI); 
//      print_mat(ZPX); 
//      print_mat(ZPZ[strata[pers]]); 
//      printf(" %lf %lf \n",1/S0,S0stratum); 
   } else  {  scl_mat_mult(scale/S0,ZPZ[0],ZPZo); scl_mat_mult(scale/S0,ZPX,ZPXo); }
...
//  scl_vec_mult(scale,zi,zi); 
  ...
  if (*betafixed==0 && *stratum==0) // {{{ computes second derivative for general cox-aalen model
...
//      for (k=0;k<*pg;k++) VE(tmpv2,k)=sqrt(VE(W2[cin],k)); 
      ...
      if (*sim>=2) replace_row(W3t[cin],s1,W3[cin]);  
      }
    ...
    for (j=0;j<*antclust;j++)  
    {
      ...
      if (*sim>=2) replace_row(W3t[j],timegroup[s],W3[j]);  
    } 
...
//	      biid[l*(*maxtimepoint)+s]=biid[l*(*maxtimepoint)+s]+VE(difX,c);
	...
	if (*sim>=2) // {{{ 
	for (k=1;k<*px+1;k++) { 
	  Rvcu[k*(*maxtimepoint)+s]=VE(VdB,k-1);
	  if (*covariance==1) {
  ...
  if (*detail>=2) Rprintf("Robust variances 2  \n"); 
    ...
    MxA(RobVbeta,SI,tmp2); MxA(SI,tmp2,RobVbeta);
...
//  for(j=0;j<*antclust;j++)  print_mat(Uti[j]); 
  ...
  if (*detail>=2) Rprintf("simulations starts  \n"); 
  ...
  if (*detail>=2) Rprintf("simulations starts for real %d \n",*sim); 
  ...
  free_mats(&dAt,&Utt,&WX,&X,&cdesX,&cdesX2,&cdesX3, &WZ,&ZP,&Z,
     &Vcov,&COV,&A,&AI,&M1,&CtVUCt, &RobVbeta,&tmp2,&dS,&S1,&S2,&SI,&VU,&VUI,
  ...
  if (*robust==1) {
    for (j=0;j<*antclust;j++) {
      if (*sim>=2) free_mat(W3t[j]); 
      if (*sim>=2) free_mat(W4t[j]); 
      if (*sim>=2) free_vec(W3[j]); 
      free_mat(W2t[j]); 
      free_mat(W2var[j]); 
      free_mat(Uti[j]); 
      free_vec(Ui[j]); 
...
////    free_mat(St[j]); 
//  } 
  for (c=0;c<(*px)*(*stratum)+1;c++) free_mat(ZPZ[c]);
